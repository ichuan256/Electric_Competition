...
static char PrintableChar(uint8_t value)
{
  return ((value >= 0x20U) && (value <= 0x7EU)) ? (char)value : '.';
}

static void FormatPayloadHex(char *line, uint8_t line_size, const uint8_t *data, uint8_t len)
{
  uint8_t pos = 0;
  uint8_t show_len = len;
  ...
  for (uint8_t i = 0; (i < show_len) && ((pos + 3U) < line_size); i++)
  {
    if (i != 0U)
    {
      line[pos++] = ' ';
    ...
    line[pos++] = HexNibble(data[i] >> 4);
    line[pos++] = HexNibble(data[i]);
  }

  if ((len > show_len) && ((pos + 3U) < line_size))
  {
    line[pos++] = '.';
    line[pos++] = '.';
    line[pos++] = '.';
...
static uint8_t IsDigitKey(char key)
{
...
{
  /* MPU will be configured later, after peripheral init */
}
