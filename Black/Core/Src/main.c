...
double Hz=900;
volatile char BoardComm_Debug_LastKey = 0;
volatile uint32_t BoardComm_Debug_KeySendCount = 0;
volatile BoardComm_Status BoardComm_Debug_LastKeySendStatus = BOARD_COMM_OK;
...
void Keypad_EventCallback(char key)
{
  ...
  BoardComm_Debug_LastKey = key;
  BoardComm_Debug_LastKeySendStatus = BoardComm_Send(BOARD_COMM_CMD_KEYPAD, &payload, 1U);
  if (BoardComm_Debug_LastKeySendStatus == BOARD_COMM_OK)
  {
    BoardComm_Debug_KeySendCount++;
  }
}
/* USER CODE END 4 */
